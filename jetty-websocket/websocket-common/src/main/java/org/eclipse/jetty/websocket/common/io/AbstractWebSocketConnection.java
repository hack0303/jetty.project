...
public abstract class AbstractWebSocketConnection extends AbstractConnection implements LogicalConnection, Connection.UpgradeTo, Dumpable
{
    ...
    private class Flusher extends FrameFlusher
    {
        ...
        public void onCompleteFailure(Throwable failure)
        {
            ...
            AbstractWebSocketConnection.this.close(failure, new DisconnectCallback());
        }
    }
    ...
    public void close(CloseInfo close, final Callback callback)
    {
        ...
        {
            if (close.getStatusCode() == StatusCode.NO_CODE || StatusCode.isTransmittable(close.getStatusCode()))
            {
    ...
    public void close(Throwable cause, Callback callback)
    {
        ...
        close(new CloseInfo(statusCode, cause.getMessage()), callback);
    }
    ...
    {
        session.callApplicationOnClose(close);
        close(close, new DisconnectCallback());
    }
    ...
    {
        if (connectionState.canWriteWebSocketFrames())
        {
            session.callApplicationOnError(timeout);
            close(new CloseInfo(StatusCode.SHUTDOWN, timeout.getMessage()), new DisconnectCallback());
            return false;
        }
        else
        {
